...
regression coefficients, and scatter plots. Built for Raghav.
"""
...
NHANES_VARS = {
    ...
    'wbc':               'WBC — White Blood Cell Count (×1000/µL)',
...
with col_left:
    ...
    dataset = st.selectbox("Dataset", ["NHANES 2001–2018", "HRS 2016 (VBS + Survey)"])
    df = nhanes if dataset.startswith("NHANES") else hrs
    var_dict = {**NHANES_VARS} if dataset.startswith("NHANES") else {**HRS_VARS, **HRS_SURVEY_VARS}

    available = [k for k in var_dict if k in df.columns]

    exposure_key = st.selectbox(
        "Exposure (X)", available,
        index=available.index('phenoage_delta') if 'phenoage_delta' in available else 0,
        format_func=lambda k: var_dict[k],
    )

    default_outcome = 'cognitive_total' if 'cognitive_total' in available else available[1]
    outcome_key = st.selectbox(
        "Outcome (Y)", [k for k in available if k != exposure_key],
        index=([k for k in available if k != exposure_key].index(default_outcome)
               if default_outcome in available else 0),
        format_func=lambda k: var_dict[k],
    )

    covariate_options = [k for k in available if k not in [exposure_key, outcome_key]]
    default_covs = [k for k in ['age','r13agey_b','sex','ragender','education','education_years']
...
with col_right:
    if not run:
        ...
        st.markdown(
            ...
            "- NHANES: Does CRP predict PhenoAge delta across NHANES cycles?\n"
            "- HRS: Does PhenoAge delta predict ADL limitations?"
        )
    else:
        ...
        analytic = df[[c for c in cols_needed if c in df.columns]].dropna().copy()
        ...
        if n < 30:
            ...
            # Partial correlation (residualize on covariates)
            ...
            reg_df.loc[len(reg_df)] = ['R²', f"{r2:.4f}"]
